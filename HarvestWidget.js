...
class HarvestWidget {
  constructor(require, options = {}) {
    ...
    let defaultOptions = {
      mode: "project", //or client // or project-client
    };
  ...
  async create() {
    ...
    try {
      ...
      this._.each(Object.keys(grouped), function (id) {
        let rows = grouped[id];
        if (type === "client") {
          client_name = rows[0].client.name;
        } else if (type === "project") {
          client_name = rows[0].project.code;
        }
  ...
  async returnWidget(items, weekTotal) {
    ...
    gradient.colors = [new Color("#f7992e"), new Color("#f56621")];
